...
function BotDashboard() {
  ...
  useEffect(() => {
    ...
    const fetchUserName = async () => {
      ...
      try {
        ...
        setUserHasBot(userData.HasBotStatus);
  ...
  }, [context.user.uid, userHasBot ]); // Dependency to re-run the effect when the user ID changes
...
<div className="text-sm text-gray-700 mt-2">{userBotName}</div>
    ...
    return(
      <div>
        // JSX code when userHasBot is false
        <div>
         <div className="flex h-screen bg-gray-100">
          {/* Left Sidebar */}
          <div className="w-64 bg-gray-800 p-4 text-white">
